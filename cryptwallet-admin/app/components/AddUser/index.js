...
class AddUser extends Component {
    ...
    role: Joi.string()
      ...
      .error(errors => {
        errors.forEach(err => {
          switch (err.type) {
            default:
              err.message = 'role can not be empty';
              break;
          }
        });
        return errors;
  ...
  submitHandler = e => {
    ...
    const { first_name, last_name, email, role, phone } = this.state;
    const newUser = {
  ...
  addUser = async user => {
    try {
      const response = await clientHttp.post('/users/add_user', user);
      console.log(response);
      if (response && response.data) toast.success(response.data.message);
    } catch (err) {
      toast.error('Error while adding new user!, Please try again laler');
    }
  };
  render() {
    return (
          ...
          <Grid container spacing={4}>
            <Grid item sm={6} xs={12}>
              <FormLabel className="formLabel">
                {this.t({ ...commonMessage.firstName })}
              </FormLabel>
              <TextField
