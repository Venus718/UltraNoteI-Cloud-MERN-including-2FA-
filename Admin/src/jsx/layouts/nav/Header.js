...
const Header = ({ onNote, userProfileData, setCurrentUser, portalURL, token }) => {
  ...
  const requestNotifications = async ()=>{
    try{
    let response = await axios.get(portalURL + 'api/notifications/', { headers: { Authorization: token.token, "Content-Type": "application/json" }});
    if(response && response.status === 200 && response.data && response.data.notifications) {
      setNotifications(response.data.notifications);
    }
  }catch(err) {
    console.log('Header.requestNotifications.catch:', err);
  }
  }
  useEffect(()=>{
